...
- Env: `UDAS_CONFIG_PATH=/path/to/config.toml`
...
touching the user's `~/.udas/config.toml`. Pass
...
`~/.udas/config.toml`, and `deepseek --model deepseek-v4-flash` is forwarded
...
- `DEEPSEEK_TASKS_DIR` (runtime task queue/artifact storage, default `~/.udas/tasks`)
...
- `DEEPSEEK_HOME` (override the base data directory; defaults to `~/.udas`)
- `DEEPSEEK_AUTOMATIONS_DIR` (override the automations storage directory; defaults to `~/.udas/automations`)
...
`~/.udas/composer_stash.jsonl`. `/stash list` shows parked
...
- `subagents.*` (optional): per-role/type model defaults for `agent_open` and
  ...
  agent is spawned.
- `skills_dir` (string, optional): defaults to `~/.udas/skills` (each skill is
...
- `mcp_config_path` (string, optional): defaults to `~/.udas/mcp.json`.
...
- `notes_path` (string, optional): defaults to `~/.udas/notes.txt` and is used by the model-visible `note` tool.
...
- `memory_path` (string, optional): defaults to `~/.udas/memory.md`.
...
- `snapshots.*` (optional): side-git workspace snapshots for file rollback:
  ...
  - snapshots live under `~/.udas/snapshots/<project_hash>/<worktree_hash>/.git` and never use the workspace's own `.git` directory
...
TUI. That means `--config` / `UDAS_CONFIG_PATH` are respected, and MCP/skills
...
- `--tools` — scaffold `~/.udas/tools/` with a `README.md` describing the
  ...
  MCP, hooks, or skills.
- `--plugins` — scaffold `~/.udas/plugins/` with a `README.md` and an
...
- `--clean` — list `~/.udas/sessions/checkpoints/latest.json` and
