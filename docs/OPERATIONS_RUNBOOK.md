...
1. Confirm binary + config:
   - `cargo run -- --version`
   - `cat ~/.udas/config.toml` (or inspect configured profile)
...
- Queue state persists to `~/.udas/sessions/checkpoints/offline_queue.json`
...
Expected behavior:
- Checkpoint stored at `~/.udas/sessions/checkpoints/latest.json`
...
Affected stores:
- sessions (`~/.udas/sessions/*.json`)
- runtime thread/turn/item records
- tasks (`~/.udas/tasks/tasks/*.json`)
...
Checks:
1. Validate `~/.udas/mcp.json` schema and server command paths
