...
(configurable via `[subagents].max_concurrent` in `~/.udas/config.toml`,
...
loaded the agent from `~/.udas/subagents.v1.json`. The parent
...
- Persisted state: `~/.udas/subagents.v1.json`. Schema version
