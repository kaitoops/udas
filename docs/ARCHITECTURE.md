...
1. Before sending user input, the TUI writes a checkpoint snapshot to `~/.udas/sessions/checkpoints/latest.json`
2. Startup remains fresh by default; prior sessions are resumed explicitly via `--resume`/`--continue` (or `Ctrl+R` in TUI)
3. While degraded/offline, new prompts are queued in-memory and mirrored to `~/.udas/sessions/checkpoints/offline_queue.json`
...
6. Agent/Yolo turns also take pre/post-turn side-git workspace snapshots under `~/.udas/snapshots/<project_hash>/<worktree_hash>/.git`; `/restore N` and `revert_turn` restore file state without changing conversation history or the user's `.git`
...
2. `task_manager.rs` persists task + queue entry under `~/.udas/tasks`
...
3. Place in `~/.udas/skills/`

### Adding Hooks

Configure in `~/.udas/config.toml`:
...
- `~/.udas/config.toml` - Main configuration
...
- `~/.udas/mcp.json` - MCP server configuration
- `~/.udas/skills/` - User skills directory
- `~/.udas/sessions/` - Session history
- `~/.udas/sessions/checkpoints/` - Crash checkpoint + offline queue persistence
- `~/.udas/snapshots/` - Side-git pre/post-turn workspace snapshots for `/restore` and `revert_turn`
- `~/.udas/tasks/` - Background task records, queue, timelines, artifacts
- `~/.udas/audit.log` - Append-only audit events for credential + approval/elevation actions
