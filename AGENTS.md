...
## Multi-Terminal Display System (Sub-Agent Display Relay)

> 首次实现于 HARNESS T-001 测试会话（2026-05-19）
> 构架文档: `docs/AGENT_DISPLAY_RELAY.md`
> 索引指针: `_DISPLAY_SYSTEM_CONTEXT.md`

当协调者通过 `agent_open` 启动子代理时，自动打开独立终端窗口以只读方式实时展示输出。

**Default model**: 子代理默认使用 `deepseek-v4-flash`（通过 `config.rs` 的 `DEFAULT_TEXT_MODEL` 常量和 `config.toml` 的 `default_text_model` 配置）。

### Session 迁移保障

每次新 Session 启动时，需确保此功能持续生效：

1. **验证编译完整性** — `cargo check --package deepseek-tui` 必须通过
2. **检查核心文件** — `crates/tui/src/tools/subagent/display_relay.rs` 必须存在且无编译错误
3. **验证集成点** — `mod.rs` 中 `display_relay` 模块声明、`SubAgentTask` 字段、`run_subagent` 参数必须完整
4. **触发预载** — 涉及以下路径时，预先载入 `_DISPLAY_SYSTEM_CONTEXT.md` 和 `docs/AGENT_DISPLAY_RELAY.md`:
   - `crates/tui/src/tools/subagent/display_relay.rs`
   - `crates/tui/src/tools/subagent/mod.rs`（display_relay 相关）
   - `crates/tui/src/main.rs`（`agent-display` 子命令）
   - 关键词：子代理输出可视化 / 多终端显示 / SubAgentDisplayRelay / agent-display
