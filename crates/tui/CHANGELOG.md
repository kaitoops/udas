...
- **Tool-result spillover and wire-dedup now share a retrieval
  ...
  (`~/.udas/tool_outputs/<id>.txt`, only above 100 KB) and the
  ...
  - The wire compactor now persists deduped content to
    `~/.udas/tool_outputs/sha_<sha>.txt` on first sighting,
...
- **AtlasCloud is now a first-class provider** (harvested from
  ...
  `provider = "atlascloud"` in `~/.udas/config.toml` (or via
...
- **MCP HTTP servers accept custom headers** for authentication
  ...
  HTTP server entry in `~/.udas/mcp.json` and the headers are
...
- **Scroll demon — alt-screen no longer drifts under parallel
  ...
  writing to `~/.udas/logs/tui-YYYY-MM-DD.log`, an fd-level
...
- **Header surfaces the runtime version chip.** A `v0.8.29` tag
  ...
  terminal width.
- **Global `~/.udas/AGENTS.md` now merges with project
...
- **Config-mutating smoke tests now isolate `UDAS_CONFIG_PATH`.**
  The command registry and web-config commit tests no longer rewrite
  the developer's real `~/.udas/config.toml` while validating
...
- **Global AGENTS.md fallback** (#1197) — when the workspace and its
  ...
  `~/.udas/AGENTS.md` before falling back to auto-generated
...
- **Provider-aware model picker and provider persistence** (#1320) —
  ...
  `~/.udas/settings.toml` so it survives restarts. The model
...
- **Lazy auto-reload of MCP pool on config-file change** (#1267 part 2) —
  ...
  `~/.udas/mcp.json` edits). `McpPool::get_or_connect` now does a
...
- **Workspace-local slash commands are now loaded (#1259)** — user command
  ...
  now discovered alongside the existing global `~/.udas/commands/`.
...
- **Cache-aware prompt diagnostics + payload optimization (#1196)** — adds
  ...
  - **Project Context Pack injected into the stable prefix by default**
    ...
    with `[context] project_pack = false` in `~/.udas/config.toml`
...
- **Global AGENTS.md fallback** - when a workspace and its parents do not
  provide project instructions, DeepSeek TUI now loads `~/.udas/AGENTS.md`
...
- **Unified slash-command namespace** (#661) — user-defined commands in
  `~/.udas/commands/` support `$1`, `$2`, `$ARGUMENTS` template
...
- **Separate tui.toml** (#657) — theme colors and keybind overrides can live in
  `~/.udas/tui.toml` alongside the main `config.toml`. *Note: file format
...
- **File @-mention frecency** (#441) — file mention suggestions learn
  from recent selections via `~/.udas/file-frecency.jsonl`.
...
- **User memory MVP** (#489–#493) — opt-in persistent note file
  ...
  - Hierarchy loader pulls `~/.udas/memory.md` (path
...
- **Session prune helper + `/sessions prune <days>`** (#406 phase-1) —
  ...
  `~/.udas/sessions/`. Skips the checkpoint subdirectory and
...
- **Tool-output spillover** (#422 + #423 + #500) — tool outputs over
  100 KiB now spill to `~/.udas/tool_outputs/<id>.txt` from the
...
- **`instructions = [...]` config array** (#454) — declare
  ...
  `~/.udas/global.md`, …) and they're concatenated into the
...
- **`load_skill` tool** (#434) — model-callable tool that takes a
  ...
  `skills` → `~/.udas/skills`). Available in Plan and
  Agent/Yolo modes.
...
- **Multi-directory skill discovery** (#432) — the system
  ...
  `<workspace>/.claude/skills` → `~/.udas/skills`. Skills
...
- **`tool.spillover` audit event** (#500 polish) — emit a
  ...
  `~/.udas/audit.log` can correlate large-output episodes
  with disk-usage growth in `~/.udas/tool_outputs/`. Fires
...
- **Prompt stash** (#440) — Ctrl+S in the composer parks the
  ...
  `~/.udas/composer_stash.jsonl` (no-op on empty composer).
...
- **`/hooks` read-only listing** (#460 MVP) — slash command
  ...
  No more `cat ~/.udas/config.toml` to debug "did my hook
...
- **`deepseek doctor` reports storage surfaces** (#422 / #440 /
  ...
  (`~/.udas/tool_outputs/`) with file count and the
  composer stash file
  (`~/.udas/composer_stash.jsonl`) with parked-draft
...
### Fixed
- Config loading now expands `~` in `UDAS_CONFIG_PATH` and `--config` paths.
- When `UDAS_CONFIG_PATH` points to a missing file, config loading now falls back to `~/.udas/config.toml` if it exists.
