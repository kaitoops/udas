...
/// when present) → [`default_skills_dir`] (`~/.udas/skills`).
...
fn skills_directories_with_home(workspace: &Path, home_dir: Option<&Path>) -> Vec<PathBuf> {
    ...
    if let Some(home) = home_dir {
        ...
        candidates.push(home.join(".udas").join("skills"));
    } else {
...
mod tests {
    ...
    fn existing_skill_dirs_orders_globals_agents_then_claude_then_deepseek() {
        ...
        let deepseek_global = tmpdir.path().join(".udas").join("skills");
    ...
    fn discover_follows_symlinked_skill_directories() {
        ...
        let skills_root = tmpdir.path().join(".udas").join("skills");
    ...
    /// `.agents/skills/` and a global skill in `~/.udas/skills/`
    ...
    fn discover_finds_both_workspace_and_global_skills() {
        ...
        assert!(
            ...
            "global-alpha from ~/.udas/skills must be discovered: {names:?}",
        );
    }
}
