...
/// Embedding mode for UDAS interference field calculations.
///
/// Controls which embedding backend is used when the UDAS engine calls
/// `embed()` on the `LlmRestorer` trait. The user can toggle between modes
/// at runtime to trade speed for semantic accuracy.
///
/// - `Fnv` (default): Fast deterministic hash, zero model loading, instant startup.
/// - `BgeM3`: Semantic embedding via remote BGE-M3 service. Requires
///   `udas-cli embed-service start` to be running. ~10 min cold start.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum EmbeddingMode {
    /// FNV-1a 256-d hash. Zero dependencies, instant. Default mode.
    #[default]
    Fnv,
    /// BGE-M3 1024-d semantic embedding via remote embedding service.
    /// Requires `udas-cli embed-service start` to be running.
    BgeM3,
}

impl EmbeddingMode {
    /// Environment variable name for initial embedding mode.
    pub const ENV_VAR: &'static str = "UDAS_EMBEDDING_MODE";

    /// Read the initial mode from the environment variable.
    /// Falls back to `Fnv` if unset or unrecognized.
    pub fn from_env() -> Self {
        match std::env::var(Self::ENV_VAR)
            ...
            .as_str()
        {
            "bge-m3" | "bge" | "on" | "true" | "1" => Self::BgeM3,
            _ => Self::Fnv,
        }
    }
    ...
    pub fn label(&self) -> &'static str {
        match self {
            Self::Fnv => "FNV-Hash (fast)",
            Self::BgeM3 => "BGE-M3 (semantic)",
        }
    }

    /// Short identifier for config files.
...
/// Global shared embedding mode state.
///
/// Uses `OnceLock` so all `DeepSeekClient` instances in the process share
/// the same embedding mode. This allows the TUI `/bge` command to toggle
/// the mode at runtime without going through the engine Op pipeline.
pub fn global_embedding_mode() -> &'static Arc<AsyncMutex<EmbeddingMode>> {
    static MODE: OnceLock<Arc<AsyncMutex<EmbeddingMode>>> = OnceLock::new();
    MODE.get_or_init(|| Arc::new(AsyncMutex::new(EmbeddingMode::from_env())))
}
...
pub struct DeepSeekClient {
    ...
    /// Cached remote embedder for the UDAS embedding service.
    /// Lazily connected on first `embed()` call in BgeM3 mode; cleared when
    /// switching back to Fnv mode.
    pub(super) remote_embedder: Arc<AsyncMutex<Option<udas_embed_service::RemoteEmbedder>>>,
    /// UDAS embedding mode: Fnv (default, fast) or BgeM3 (semantic, requires service).
    /// Toggled at runtime via `set_embedding_mode()`.
    pub(super) embedding_mode: Arc<AsyncMutex<EmbeddingMode>>,
}
...
impl Clone for DeepSeekClient {
    fn clone(&self) -> Self {
        Self {
            ...
            remote_embedder: self.remote_embedder.clone(),
...
impl DeepSeekClient {
    ...
    pub fn new(config: &Config) -> Result<Self> {
        ...
        Ok(Self {
            ...
            remote_embedder: Arc::new(AsyncMutex::new(None)),
            embedding_mode: global_embedding_mode().clone(),
        })
    }
...
impl DeepSeekClient {
    ...
    // 鈹€鈹€鈹€ UDAS Embedding Mode Control 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€

    /// Get the current UDAS embedding mode.
    ///
    /// Returns `Fnv` (default, fast hash) or `BgeM3` (semantic, requires service).
    pub async fn udas_embedding_mode(&self) -> EmbeddingMode {
        *self.embedding_mode.lock().await
    }

    /// Set the UDAS embedding mode.
    ///
    /// When switching to `BgeM3`, the next `embed()` call will attempt to
    /// connect to the embedding service. When switching to `Fnv`, any cached
    /// remote embedder connection is cleared.
    ///
    /// Use `/bge` in the TUI or `UDAS_EMBEDDING_MODE` env var to control this.
    pub async fn set_udas_embedding_mode(&self, mode: EmbeddingMode) {
        let mut guard = self.embedding_mode.lock().await;
        let old = *guard;
        *guard = mode;
        drop(guard);

        if mode == EmbeddingMode::Fnv && old == EmbeddingMode::BgeM3 {
            // Clear cached remote embedder when switching back to FNV
            *self.remote_embedder.lock().await = None;
            tracing::info!("UDAS embedding mode: BgeM3 -> Fnv (cleared remote embedder cache)");
        } else {
            tracing::info!("UDAS embedding mode: {:?} -> {:?}", old, mode);
        }
    }

    /// Toggle UDAS embedding mode between Fnv and BgeM3.
    /// Returns the new mode after toggling.
    pub async fn toggle_udas_embedding_mode(&self) -> EmbeddingMode {
        let current = self.udas_embedding_mode().await;
        let new_mode = match current {
            EmbeddingMode::Fnv => EmbeddingMode::BgeM3,
            EmbeddingMode::BgeM3 => EmbeddingMode::Fnv,
        };
        self.set_udas_embedding_mode(new_mode).await;
        new_mode
    }
}
