...
//! daily-rolling file under `~/.udas/logs/`, and (on Unix) redirects the
...
//!      `~/.udas/logs/tui-YYYY-MM-DD.log` so `tracing::warn!` /
...
fn log_directory() -> Option<PathBuf> {
    ...
    {
        return Some(home.join(".udas").join("logs"));
    }
    if let Some(userprofile) = std::env::var_os("USERPROFILE").map(PathBuf::from)
        && !userprofile.as_os_str().is_empty()
    {
        return Some(userprofile.join(".udas").join("logs"));
    }
    dirs::home_dir().map(|h| h.join(".udas").join("logs"))
}
...
mod tests {
    ...
    fn log_directory_prefers_home() {
        ...
        assert_eq!(resolved, tmp.path().join(".udas").join("logs"));
