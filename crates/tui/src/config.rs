...
pub const MAX_SUBAGENTS: usize = 25;
...
pub enum SearchProvider {
    /// Multi-engine HTML scraping with Tavily-first fallback.
    /// Tries Tavily first if api_key is available, then falls back to
    /// domestic Chinese engines (Baidu, Sogou, 360, Toutiao) then international.
    #[serde(alias = "multi")]
    #[default]
    Multi,
...
impl Config {
    ...
    /// `~/.udas/config.toml` for ollama / vllm / openai-compat servers
            ...
            ApiProvider::Deepseek | ApiProvider::DeepseekCN => anyhow::bail!(
                "DeepSeek API key not found.\n\
                 ...
                 Alternatives:\n\
                   ...
                   • api_key = \"<your-key>\"  in ~/.udas/config.toml"
            ),
            ApiProvider::NvidiaNim => anyhow::bail!(
                "NVIDIA NIM API key not found. Run 'deepseek auth set --provider nvidia-nim', \
                 set NVIDIA_API_KEY/NVIDIA_NIM_API_KEY, or save api_key in ~/.udas/config.toml \
...
fn home_config_path() -> Option<PathBuf> {
    effective_home_dir().map(|home| home.join(".udas").join("config.toml"))
}
...
fn env_config_path() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("UDAS_CONFIG_PATH") {
...
fn default_managed_config_path() -> Option<PathBuf> {
    ...
    #[cfg(not(unix))]
    {
        effective_home_dir().map(|home| home.join(".udas").join("managed_config.toml"))
    }
}
...
fn default_requirements_path() -> Option<PathBuf> {
    ...
    #[cfg(not(unix))]
    {
        effective_home_dir().map(|home| home.join(".udas").join("requirements.toml"))
    }
}
...
fn default_skills_dir() -> Option<PathBuf> {
    effective_home_dir().map(|home| home.join(".udas").join("skills"))
}

fn default_mcp_config_path() -> Option<PathBuf> {
    effective_home_dir().map(|home| home.join(".udas").join("mcp.json"))
}

fn default_notes_path() -> Option<PathBuf> {
    effective_home_dir().map(|home| home.join(".udas").join("notes.txt"))
}

fn default_memory_path() -> Option<PathBuf> {
    effective_home_dir().map(|home| home.join(".udas").join("memory.md"))
}
...
pub enum SavedCredential {
    ...
    /// (`"system keyring"`, `"file-based (~/.udas/secrets/)"`).
...
///
/// **Dual-write strategy (#593):** writes to `~/.udas/config.toml`
...
/// `[providers.<name>] api_key = "..."` to `~/.udas/config.toml`.
...
mod tests {
    ...
    impl EnvGuard {
        fn new(home: &Path) -> Self {
            ...
            let config_path = home.join(".udas").join("config.toml");
            let config_str = OsString::from(config_path.as_os_str());
            let home_prev = env::var_os("HOME");
            let userprofile_prev = env::var_os("USERPROFILE");
            let deepseek_config_prev = env::var_os("UDAS_CONFIG_PATH");
            ...
            unsafe {
                ...
                env::set_var("UDAS_CONFIG_PATH", &config_str);
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            ...
            unsafe {
                ...
                Self::restore_var("UDAS_CONFIG_PATH", self.deepseek_config_path.take());
    ...
    fn save_api_key_writes_config_file_under_cfg_test() -> Result<()> {
        ...
        let expected = temp_root.join(".udas").join("config.toml");
    ...
    fn ensure_config_file_exists_creates_first_run_template() -> Result<()> {
        ...
        assert_eq!(created, temp_root.join(".udas").join("config.toml"));
    ...
    fn workspace_trust_round_trips_through_global_config() -> Result<()> {
        ...
        assert_eq!(saved, temp_root.join(".udas").join("config.toml"));
        assert!(is_workspace_trusted(&workspace));
        assert!(!crate::tui::onboarding::needs_trust(&workspace));
        assert!(
            !workspace.join(".udas").exists(),
    ...
    fn workspace_trust_reads_existing_projects_table() -> Result<()> {
        ...
        let workspace = temp_root.join("project");
        fs::create_dir_all(&workspace)?;
        let config_path = temp_root.join(".udas").join("config.toml");
    ...
    fn clear_api_key_strips_root_and_provider_scoped_keys() -> Result<()> {
        ...
        let config_dir = temp_root.join(".udas");
...
"#,
    ...
    fn test_tilde_expansion_in_paths() -> Result<()> {
        ...
        let config = Config {
            skills_dir: Some("~/.udas/skills".to_string()),
            ..Default::default()
        };
        let expected_skills = temp_root.join(".udas").join("skills");
    ...
    fn test_load_uses_tilde_expanded_deepseek_config_path() -> Result<()> {
        ...
        // Safety: test-only environment mutation guarded by a global mutex.
        unsafe {
            env::set_var("UDAS_CONFIG_PATH", "~/.custom-deepseek/config.toml");
        }
    ...
    fn test_load_falls_back_to_home_config_when_env_path_missing() -> Result<()> {
        ...
        let home_config = temp_root.join(".udas").join("config.toml");
