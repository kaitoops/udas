...
/// Persist `tui.status_items` to `~/.udas/config.toml` without disturbing
...
/// Resolve the path to `~/.udas/config.toml` (or
/// `$UDAS_CONFIG_PATH`). Mirrors what `Config::load` accepts so we
/// never write to a different file than the one we read.
pub(super) fn config_toml_path() -> anyhow::Result<PathBuf> {
    use anyhow::Context;
    if let Ok(env) = std::env::var("UDAS_CONFIG_PATH") {
    ...
    Ok(home.join(".udas").join("config.toml"))
}
...
mod tests {
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            ...
            if let Some(value) = self.deepseek_config_path.take() {
                // Safety: test-only environment mutation guarded by a global mutex.
                unsafe {
                    env::set_var("UDAS_CONFIG_PATH", value);
                }
            } else {
                // Safety: test-only environment mutation guarded by a global mutex.
                unsafe {
                    env::remove_var("UDAS_CONFIG_PATH");
                }
            }
        }
    }
    ...
    fn persist_status_items_preserves_existing_unrelated_keys() {
        ...
        let path = temp_root.join(".udas").join("config.toml");
