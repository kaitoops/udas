...
mod tests {
    ...
    impl EnvGuard {
        fn new(home: &Path) -> Self {
            ...
            unsafe {
                ...
                env::set_var("UDAS_CONFIG_PATH", config_path.as_os_str());
            }
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            ...
            restore_env("UDAS_CONFIG_PATH", self.deepseek_config_path.take());
        }
    }
    ...
    fn network_allow_extracts_host_from_url() {
        ...
        assert!(!result.is_error, "{:?}", result.message);
        let body = fs::read_to_string(home.join(".udas").join("config.toml")).unwrap();
        assert!(body.contains("allow = [\"github.com\"]"), "{body}");
    }
