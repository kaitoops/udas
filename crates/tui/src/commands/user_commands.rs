//! User-defined slash commands from `~/.udas/commands/<name>.md` and
...
//! 4. `~/.udas/commands/`            (user-global, lowest)
...
/// Path to the global user commands directory: `~/.udas/commands/`.
fn global_commands_dir() -> PathBuf {
    let home = dirs::home_dir().unwrap_or_else(|| PathBuf::from("~"));
    home.join(".udas").join("commands")
}
...
fn commands_dirs(workspace: Option<&Path>) -> Vec<PathBuf> {
    ...
    if let Some(ws) = workspace {
        dirs.push(ws.join(".udas").join("commands"));
...
mod tests {
    ...
    fn load_user_commands_scans_workspace_local_dir() {
        ...
        let ws = tmp.path();
        let cmds_dir = ws.join(".udas").join("commands");
    ...
    fn load_user_commands_without_workspace_falls_back_to_global_only() {
        // When no workspace is passed, only the global ~/.udas/commands/
