...
pub fn execute(cmd: &str, app: &mut App) -> CommandResult {
    ...
    match command {
        ...
        // UDAS embedding mode toggle
        "bge" => {
            let mode_arc = crate::client::global_embedding_mode();
            match mode_arc.try_lock() {
                Ok(mut guard) => {
                    use crate::client::EmbeddingMode;
                    let old = *guard;
...
/// Persist the user's chosen footer items to `~/.udas/config.toml` under
...
mod tests {
    ...
    impl ConfigPathGuard {
        fn new(config_path: &Path) -> Self {
            ...
            let previous = std::env::var_os("UDAS_CONFIG_PATH");
            // Safety: test-only environment mutation guarded by a global mutex.
            unsafe {
                std::env::set_var("UDAS_CONFIG_PATH", config_path);
            }
            Self {
                previous,
                _lock: lock,
        ...
        fn drop(&mut self) {
            ...
            unsafe {
                if let Some(previous) = self.previous.take() {
                    std::env::set_var("UDAS_CONFIG_PATH", previous);
                } else {
    ...
    fn create_isolated_test_app() -> (App, tempfile::TempDir, ConfigPathGuard) {
        ...
        let config_path = workspace.join(".udas").join("config.toml");
