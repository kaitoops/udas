...
//!
//! Storage: `~/.udas/workspace-trust.json`. The file is a JSON object
...
fn trust_file_path() -> Option<PathBuf> {
    dirs::home_dir().map(|home| home.join(".udas").join(TRUST_FILE_NAME))
}
...
mod tests {
    ...
    /// Set up an isolated fake `~/.udas/workspace-trust.json` location.
    ...
    fn isolated_trust_path() -> (TempDir, PathBuf) {
        ...
        let trust_path = tmp.path().join(".udas").join("workspace-trust.json");
        (tmp, trust_path)
    }
