...
/// in `~/.udas/config.toml` doesn't fail the launch. Empty input
...
pub fn system_prompt_for_mode_with_context_skills_session_and_approval(
    ...
    // `~/.agents/skills` / `~/.udas/skills` so skills installed for any
...
mod tests {
    ...
    fn handoff_artifact_is_prepended_to_system_prompt_when_present() {
        ...
        let handoff_dir = workspace.join(".udas");
