//! Schema migration framework for `~/.udas/` persisted records.
//!
...
pub mod registry {
    ...
    /// Sessions: `~/.udas/sessions/<id>.json` and the latest
    /// checkpoint at `~/.udas/sessions/checkpoints/latest.json`.
    pub struct SessionMigration;
    impl SchemaMigration for SessionMigration {
        const CURRENT_VERSION: u32 = 1;
        const DOMAIN: &'static str = "session";
    ...
    /// Offline queue: `~/.udas/sessions/checkpoints/offline_queue.json`.
    ...
    /// Durable tasks under `~/.udas/tasks/`.
