...
impl SessionManager {
    ...
    /// Create a `SessionManager` using the default location (~/.udas/sessions)
    ...
    /// user may have rsynced their `~/.udas` between machines and
    /// fs mtimes can lie.
...
/// Resolve the default session directory path (`~/.udas/sessions`).
pub fn default_sessions_dir() -> std::io::Result<PathBuf> {
    let home = dirs::home_dir().ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "Home directory not found")
    })?;
    Ok(home.join(".udas").join("sessions"))
}
