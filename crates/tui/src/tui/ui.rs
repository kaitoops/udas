...
pub async fn run_tui(config: &Config, options: TuiOptions) -> Result<()> {
    ...
    // in `~/.udas/logs/tui-YYYY-MM-DD.log` instead. The guard is held
...
pub(crate) fn apply_engine_error_to_app(
    ...
    ) && app.api_key_env_only
    {
        ...
        app.onboarding = OnboardingState::ApiKey;
        app.status_message = Some(
            "The API key from DEEPSEEK_API_KEY was rejected. Paste a valid key to save it to ~/.udas/config.toml, or update the environment variable.".to_string(),
        );
        return;
    }
    if !recoverable {
...
/// `~/.udas/settings.toml` so it survives a restart, push the change to
...
/// Persist the typed API key to `~/.udas/config.toml`, refresh the
