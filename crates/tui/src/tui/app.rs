...
pub enum OnboardingState {
    ...
    /// land in `~/.udas/settings.toml` via `Settings::set("locale", …)`.
    Language,
    ApiKey,
    TrustDirectory,
    Tips,
...
pub struct App {
    ...
    /// `tui.status_items` in `~/.udas/config.toml` at startup; mutated
...
impl App {
    ...
    /// Persists the value to `~/.udas/settings.toml` and immediately
    ...
    /// Locale tag currently persisted in `~/.udas/settings.toml` (or
...
mod tests {
    ...
    fn settings_default_provider_auth_check_uses_provider_scoped_key() {
        ...
        .expect("settings");
        let _config_path = EnvVarGuard::set("UDAS_CONFIG_PATH", &config_path);
