...
//! encoded as PNG and persisted under `~/.udas/clipboard-images/` so the
...
impl ClipboardHandler {
    ...
    ///
    /// `workspace` is used as a fallback location when `~/.udas/` cannot
...
/// `~/.udas/clipboard-images/` so the path is stable across worktrees and
...
fn clipboard_images_dir(workspace: &Path) -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        return home.join(".udas").join("clipboard-images");
    }
