...
mod tests {
    ...
    fn build_document_reflects_cost_currency_from_settings() {
        ...
        fs::create_dir_all(temp_root.join(".udas")).expect("config dir");
        let config_path = temp_root.join(".udas").join("config.toml");
        fs::write(&config_path, "").expect("seed config");
        fs::write(
            temp_root.join(".udas").join("settings.toml"),
            r#"
cost_currency = "cny"
"#,
        )
        ...
        let old_config_path = std::env::var_os("UDAS_CONFIG_PATH");
        ...
        unsafe {
            if let Some(value) = old_config_path {
                std::env::set_var("UDAS_CONFIG_PATH", value);
            } else {
