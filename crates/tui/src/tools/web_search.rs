...
// Multi Search Engine hosts
const GOOGLE_HOST: &str = "www.google.com";
const YAHOO_HOST: &str = "search.yahoo.com";
const STARTPAGE_HOST: &str = "www.startpage.com";
const ECOSIA_HOST: &str = "www.ecosia.org";
const QWANT_HOST: &str = "www.qwant.com";
const MOJEEK_HOST: &str = "www.mojeek.com";
const SEARX_HOST: &str = "searx.be";
const YANDEX_HOST: &str = "yandex.com";
const BAIDU_HOST: &str = "www.baidu.com";
const SOGOU_HOST: &str = "www.sogou.com";
const SO_HOST: &str = "www.so.com";
const TOUTIAO_HOST: &str = "so.toutiao.com";
    ...
    async fn execute(&self, input: Value, context: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        match context.search_provider {
            SearchProvider::Multi => {
                // Multi-engine: Tavily-first strategy with domestic engine fallback
                return self
                    .run_multi_search(&query, max_results, timeout_ms, context)
                    .await;
            }
...
impl WebSearchTool {
    ...
    /// Multi-engine search: Tavily-first with domestic Chinese engine fallback.
    /// Tries Tavily first if API key is available, then falls back to domestic
    /// engines (Baidu, Sogou, 360, Toutiao), then international engines.
    async fn run_multi_search(
        ...
        for (name, host, parser) in engines {
            if check_policy(decider, host).is_err() {
                continue;
            }
            ...
            let url = match name {
                ...
                "startpage" => format!("https://www.startpage.com/sp/search?q={encoded}"),
                "ecosia" => format!("https://www.ecosia.org/search?q={encoded}"),
                "qwant" => format!("https://www.qwant.com/?q={encoded}"),
                "mojeek" => format!("https://www.mojeek.com/search?q={encoded}"),
                "searx" => format!("https://searx.be/search?q={encoded}"),
                "yandex" => format!("https://yandex.com/search/?text={encoded}"),
                "baidu" => format!("https://www.baidu.com/s?wd={encoded}"),
                "sogou" => format!("https://www.sogou.com/web?query={encoded}"),
                "so" => format!("https://www.so.com/s?q={encoded}"),
                "toutiao" => format!("https://so.toutiao.com/search?keyword={encoded}"),
                _ => continue,
            };
            ...
            let results = parser(&body, max_results);
            if !results.is_empty() {
                let response = WebSearchResponse {
                    query: query.to_string(),
        ...
        let response = WebSearchResponse {
            ...
            count: 0,
            message: "No results found from any search engine".to_string(),
            results: Vec::new(),
        };
...
fn parse_yahoo_results(html: &str, max_results: usize) -> Vec<WebSearchEntry> {
    let mut results = Vec::new();
...
fn parse_yandex_results(html: &str, max_results: usize) -> Vec<WebSearchEntry> {
    let mut results = Vec::new();
...
/// Parse 360 Search (so.com) results from HTML
fn parse_so_results(html: &str, max_results: usize) -> Vec<WebSearchEntry> {
    let mut results = Vec::new();
