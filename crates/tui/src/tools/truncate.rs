...
//! `~/.udas/tool_outputs/<sanitised-id>.txt`. The id is the tool
...
/// Name of the spillover directory under `~/.udas/`.
...
/// startup to keep `~/.udas/tool_outputs/` from growing without
...
/// Resolve `~/.udas/tool_outputs/`. Returns `None` if the home
...
pub fn spillover_root() -> Option<PathBuf> {
    ...
    Some(dirs::home_dir()?.join(".udas").join(SPILLOVER_DIR_NAME))
}
...
/// content to a sibling file under `~/.udas/tool_outputs/`,
...
///
/// The legacy `~/.udas/tool_outputs/<tool-id>.txt` file is still written
...
/// `~/.udas/sessions/<session-id>/artifacts/`, and the inline tool result
...
/// user's real `~/.udas/` directory. This uses explicit test hooks instead
    ...
    F: FnOnce() -> R,
{
    ...
    // artifacts.rs tests.
    let prior_spillover =
        set_test_spillover_root(Some(home.join(".udas").join(SPILLOVER_DIR_NAME)));
    let prior_artifacts = crate::artifacts::set_test_artifact_sessions_root(Some(
        home.join(".udas").join("sessions"),
    ));
    ...
    fn with_test_home_overrides_storage_roots_without_home_resolution() {
        ...
        with_test_home(tmp.path(), || {
            assert_eq!(
                ...
                Some(tmp.path().join(".udas").join("tool_outputs").as_path())
            );
            assert_eq!(
