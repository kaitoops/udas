...
pub struct ToolContext {
    ...
    /// fall outside `workspace`. Loaded from `~/.udas/workspace-trust.json`
...
impl ToolContext {
    ...
    pub fn new(workspace: impl Into<PathBuf>) -> Self {
        ...
        let notes_path = workspace.join(".udas").join("notes.md");
        let mcp_config_path = workspace.join(".udas").join("mcp.json");
        Self {
            workspace,
            shell_manager,
            trust_mode: false,
    ...
    /// `~/.udas/workspace-trust.json`). See [`Self::resolve_path`] for
