...
mod tests {
    ...
    /// Serialises tests that mutate `UDAS_CONFIG_PATH` so they don't
    ...
    impl ConfigPathEnvGuard {
        fn capture() -> Self {
            Self {
                prior: std::env::var_os("UDAS_CONFIG_PATH"),
            }
        }
    }
    impl Drop for ConfigPathEnvGuard {
        fn drop(&mut self) {
            ...
            match &self.prior {
                Some(v) => unsafe { std::env::set_var("UDAS_CONFIG_PATH", v) },
                None => unsafe { std::env::remove_var("UDAS_CONFIG_PATH") },
            }
        }
    }
