...
struct SpawnRequest {
    ...
    /// HARNESS agent role override: "planner" | "evaluator" | "generator".
    harness_role: Option<String>,
    /// Path to an external DeepSeek binary for out-of-process execution.
    /// When set, the child runs as a separate process rather than an
    /// in-process sub-agent. Used for multi-binary team dispatch.
...
/// `[runtime] max_spawn_depth = N` in `~/.udas/config.toml`.
...
impl SubAgentManager {
    ...
    pub(crate) fn spawn_background_with_assignment_options(
        ...
        // Set up display relay for external terminal visualisation.
        // Best-effort: failure to create or launch is non-fatal.
        // If too many windows are already open, we still write the relay
        // file but skip launching a new terminal window.
        ...
        if let Some(ref mut r) = display_relay {
            r.write_status(&format!(
                "spawned (type: {}, steps: {max_steps})",
                agent_type.as_str()
            ));
            let active = display_relay::active_window_count();
            if active < display_relay::MAX_DISPLAY_WINDOWS {
                r.launch_terminal();
            }
        }
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "harness_role": {
                    "type": "string",
                    "enum": ["planner", "evaluator", "generator"],
                    "description": "HARNESS agent role. When set, injects the agent's constraint rules into the system prompt."
                },
                "binary_path": {
                    "type": "string",
                    "description": "Path to an external DeepSeek binary. When set, spawns as a separate OS process instead of an in-process sub-agent."
                },
                "cwd": {
                    "type": "string",
...
impl ToolSpec for AgentSpawnTool {
    ...
    async fn execute(&self, input: Value, _context: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        let mut configured_model = match spawn_request.model.clone() {
        ...
        // All sub-agents default to V4 Flash to minimise API costs.
        // Only override when the user explicitly specified a model or when
        // auto_model routing is active.
        ...
        // ── External binary execution (Phase 3: multi-binary team dispatch) ──
        //
        // When `binary_path` is set, launch the specified DeepSeek binary as a
        // separate OS process instead of spawning an in-process sub-agent.
        if let Some(ref bin_path) = spawn_request.binary_path {
            use std::process::{Command, Stdio};

            let bin_path = if bin_path.is_absolute() {
                bin_path.clone()
            } else {
                self.runtime.context.workspace.join(bin_path)
            };

            let mut cmd = Command::new(&bin_path);
            cmd.arg("-p").arg(&effective_prompt);
            cmd.env("DEEPSEEK_MODEL", &effective_model);
            ...
            let output = cmd
                ...
                .map_err(|e| {
                    ToolError::execution_failed(format!(
                        "Failed to execute external binary {}: {e}",
                        bin_path.display()
                    ))
                })?;
            ...
            let summary = if exit_code == 0 {
                format!("External process completed (exit code: {exit_code})")
            } else {
                format!("External process failed (exit code: {exit_code})\nstderr: {stderr}")
            };
...
/// full role library — TOML overlays from `~/.udas/roles/`, the
...
fn build_subagent_system_prompt(
    ...
    const LANG_PREF: &str = "\
Language preference: Use Chinese (Simplified) for all semantic content \
(status updates, reasoning summaries, conclusions, error messages). \
Keep code output in English. If this instruction increases token usage, \
ignore it and output in your native language.";

    let base = agent_type.system_prompt();
    let base = format!("{LANG_PREF}\n\n{base}");
...
/// Tracks a tool use block being assembled during streaming.
...
/// Tracks what kind of content block is currently being streamed.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
async fn run_subagent_task(task: SubAgentTask) {
    ...
    // text.
    // Write final status to the display relay.
    match &result {
        Ok(res) => {
            let summary = summarize_subagent_result(res);
            if let Some(ref r) = relay {
                r.write_complete(&summary);
            }
        }
        Err(err) => {
            if let Some(ref r) = relay {
                r.write_failed(&err.to_string());
            }
        }
    }
...
async fn run_subagent(
    ...
    relay: Option<&display_relay::SubAgentDisplayRelay>,
) -> Result<SubAgentResult> {
    ...
    for _step in 0..max_steps {
        ...
        // Stream the API response — each delta is forwarded to the display
        // relay in real-time so the external terminal shows token-by-token
        // output instead of waiting for the complete response.
        //
        // We use `create_message_stream` instead of `create_message` to get
        // incremental deltas. The stream is consumed until MessageStop or
        // cancellation, building up the final MessageResponse from events.
        ...
        let mut stream_content: Vec<ContentBlock> = Vec::new();
        ...
        let mut stream = tokio::time::timeout(
            STEP_API_TIMEOUT,
            runtime.client.create_message_stream(request),
        )
        .await
        .map_err(|_| anyhow!("API stream timed out after {}s", STEP_API_TIMEOUT.as_secs()))??;
        ...
        loop {
            ...
            if runtime.cancel_token.is_cancelled() {
                emit_agent_progress(
                    ...
                    runtime.mailbox.as_ref(),
                    &agent_id,
                    format!("step {steps}/{max_steps}: cancelled mid-stream"),
                );
                if let Some(mb) = runtime.mailbox.as_ref() {
                    let _ = mb.send(MailboxMessage::Cancelled {
                        agent_id: agent_id.clone(),
            ...
            match event {
                ...
                StreamEvent::ContentBlockStop { index } => {
                    ...
                    if let Some(&tool_idx) = current_tool_indices.get(&index) {
                        ...
                        stream_content.push(ContentBlock::ToolUse {
                            id: ts.id.clone(),
                            name: ts.name.clone(),
                            input,
                            caller: None,
                        });
                    } else if let Some(kind) = current_block_kind.take() {
                        match kind {
                            ContentBlockKind::Text => {
                                if !stream_text.is_empty() {
                                    stream_content.push(ContentBlock::Text {
                                        text: stream_text.clone(),
                                        cache_control: None,
                                    });
                                }
                            }
        ...
        // Assemble the final response from the stream
        let response = crate::models::MessageResponse {
            id: String::new(),
        ...
        // Extract text result and tool uses from the assembled content
...
fn parse_spawn_request(input: &Value) -> Result<SpawnRequest, ToolError> {
    ...
    let harness_role = input
        .get("harness_role")
        .and_then(|v| v.as_str())
        .map(str::trim)
        .filter(|s| !s.is_empty())
        .map(|s| s.to_ascii_lowercase())
        .filter(|s| matches!(s.as_str(), "planner" | "evaluator" | "generator"));
