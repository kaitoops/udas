...
//! switches are stored separately at ~/.udas/tui.toml. See [`TuiPrefs`].
...
impl TuiPrefs {
    ...
    /// `UDAS_CONFIG_PATH` environment variable (the parent directory of
    /// the pointed-to config is used instead of `~/.udas`).
    pub fn path() -> Result<PathBuf> {
        ...
        if let Ok(config_path) = std::env::var("UDAS_CONFIG_PATH") {
        ...
        Ok(home.join(".udas").join("tui.toml"))
    }

    /// Load TUI preferences from `~/.udas/tui.toml`.
    ///
    ...
    /// Save TUI preferences to `~/.udas/tui.toml`, creating the
    /// `~/.udas` directory if needed.
...
impl Settings {
    ...
    pub fn path() -> Result<PathBuf> {
        ...
        // used for config (UDAS_CONFIG_PATH points at config.toml; the
...
mod tests {
    ...
    /// Serialise tests that mutate `UDAS_CONFIG_PATH` through this guard
    ...
    fn tui_prefs_load_returns_defaults_when_file_absent() {
        ...
        // SAFETY: cleanup under the guard.
        unsafe {
            std::env::remove_var("UDAS_CONFIG_PATH");
        }
