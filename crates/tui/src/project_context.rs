...
fn load_project_context_with_parents_and_home(
    ...
    // Always check `~/.udas/AGENTS.md` so user-wide preferences
...
/// Combine `~/.udas/AGENTS.md` (global, user-wide preferences) with a
...
fn auto_generate_context(workspace: &Path) -> Option<String> {
    let deepseek_dir = workspace.join(".udas");
...
fn check_trust_status(workspace: &Path) -> bool {
    ...
    let trust_markers = [
        workspace.join(".udas").join("trusted"),
        workspace.join(".udas").join("trust.json"),
    ];
...
mod tests {
    ...
    fn test_load_project_context_hidden_dir() {
        let tmp = tempdir().expect("tempdir");
        let hidden_dir = tmp.path().join(".udas");
    ...
    fn test_check_trust_status() {
        ...
        // Create trust marker
        let deepseek_dir = tmp.path().join(".udas");
    ...
    fn project_context_pack_ignores_agent_state_and_binary_noise() {
        ...
        fs::create_dir_all(tmp.path().join(".udas").join("state")).expect("mkdir state");
        fs::write(
            tmp.path()
                .join(".udas")
                .join("state")
    ...
    fn test_load_global_agents_when_project_has_no_context() {
        ...
        let global_dir = home.path().join(".udas");
    ...
    fn test_local_and_global_agents_merge_when_both_exist() {
        // #1157: when both `~/.udas/AGENTS.md` and a project AGENTS.md
