...
#[command(
    ...
    author,
    version = env!("UDAS_BUILD_VERSION"),
...
struct Cli {
    ...
    /// Skip loading project-level config from $WORKSPACE/.udas/config.toml
...
enum Commands {
    ...
    /// Read-only display of a sub-agent's output in a terminal window.
    /// Launched automatically when a sub-agent is spawned; can also be
    /// invoked manually to re-attach to a running or completed agent.
    AgentDisplay {
        /// Agent ID (e.g. "agent_03f9fe86") or session name to display.
        #[arg(value_name = "AGENT_ID")]
        agent_id: String,
    },
    /// Run the UDAS (Unitary Disk Active Search) pipeline
    Udas(UdasArgs),
    /// Run memory restoration collision collapse (环境信号驱动干涉坍缩)
    UdasCollapse(UdasCollapseArgs),
}
...
async fn main() -> Result<()> {
    ...
    // to ~/.udas/crashes/ even if the panic happens before tokio is up,
    ...
    std::panic::set_hook(Box::new(move |panic_info| {
        ...
        if let Some(home) = dirs::home_dir() {
            let crash_dir = home.join(".udas").join("crashes");
    ...
    if let Some(command) = cli.command.clone() {
        return match command {
            ...
            Commands::AgentDisplay { agent_id } => run_agent_display(&agent_id),
            Commands::Udas(args) => {
                let config = load_config_from_cli(&cli)?;
                run_udas(&config, args).await
            }
            Commands::UdasCollapse(args) => {
                let config = load_config_from_cli(&cli)?;
                run_udas_collapse(&config, args).await
            }
        };
    }
...
/// Read-only display of a sub-agent's output in the current terminal.
///
/// Launched automatically by `SubAgentDisplayRelay::launch_terminal` when a
/// sub-agent is spawned. The display client follows the output file and
/// renders ANSI-styled content. It is read-only: all keyboard input is
/// ignored.
fn run_agent_display(agent_id: &str) -> Result<()> {
    ...
    // Without this, ANSI color codes render as raw escape sequences.
    #[cfg(windows)]
    {
    ...
    // Resolve the display directory.
    let home = dirs::home_dir().unwrap_or_else(|| PathBuf::from("."));
    let display_dir = home.join(".udas").join("agent-display").join(agent_id);
    let output_path = display_dir.join("output.log");
    let done_path = display_dir.join("done");
    ...
    // ── Read-only display loop ──────────────────────────────────────────

    // We read the file from the beginning each time and display all content.
    // ANSI escape codes written by the relay render directly in the terminal.
    ...
    loop {
        ...
        if done_path.exists() {
            ...
            while remaining_min > 0 {
                print!("\r\x1b[2mAuto-close in {:02}:00...  \x1b[0m", remaining_min);
                use std::io::Write;
                let _ = std::io::stdout().flush();
            ...
            println!("\r\x1b[2mAuto-closing...                  \x1b[0m");
            return Ok(());
        }
...
fn plugins_readme_template() -> &'static str {
    "# Local plugins\n\n\
     ...
     experiments without touching `~/.udas/skills/`.\n\n\
...
fn run_setup_status(config: &Config, workspace: &Path) -> Result<()> {
    ...
    match resolve_api_key_source(config) {
        ...
        ApiKeySource::Missing => {
            ...
            println!(
                "  {} api_key: missing  (set {env_var} or `[providers.{}].api_key` in ~/.udas/config.toml; or run `{login_hint}`)",
...
async fn run_doctor(config: &Config, workspace: &Path, config_path_override: Option<&Path>) {
    ...
    println!("  deepseek-tui: {}", env!("UDAS_BUILD_VERSION"));
    ...
    println!("  · credential precedence: ~/.udas/config.toml, OS keyring, then env");
    ...
    let has_api_key = if config.deepseek_api_key().is_ok() {
        ...
        println!(
            "    Run 'deepseek auth set --provider <name>' to save a key to ~/.udas/config.toml."
        );
        false
    };
    ...
    let stash_path = dirs::home_dir().map(|h| h.join(".udas").join("composer_stash.jsonl"));
...
fn run_doctor_json(
    ...
    let report = json!({
        ...
        "storage": {
            ...
            "stash": {
                "path": dirs::home_dir()
                    .map(|h| h.join(".udas").join("composer_stash.jsonl").display().to_string())
                    .unwrap_or_default(),
                "present": dirs::home_dir()
                    .map(|h| h.join(".udas").join("composer_stash.jsonl"))
...
fn doctor_timeout_recovery_lines(config: &Config) -> Vec<String> {
    ...
    match config.api_provider() {
        ...
        {
            lines.push(
                "If this is a custom DeepSeek-compatible endpoint, set its HTTPS base URL in ~/.udas/config.toml and rerun `deepseek doctor`."
                    .to_string(),
            );
        }
        crate::config::ApiProvider::Deepseek | crate::config::ApiProvider::DeepseekCN => {
...
/// `~/.udas/config.toml` or `--mouse-capture`.
...
/// Load project-level config from `$WORKSPACE/.udas/config.toml` and
...
fn merge_project_config(config: &mut Config, workspace: &Path) {
    let path = workspace.join(".udas").join("config.toml");
    ...
    // `<workspace>/.udas/config.toml` could otherwise:
        ...
        if table.contains_key(*key) {
            eprintln!(
                "warning: project-scope config key `{key}` is ignored — \
                 set it in `~/.udas/config.toml` instead. \
...
async fn run_interactive(
    ...
    // Merge project-level config from $WORKSPACE/.udas/config.toml
...
#[derive(Args, Debug, Clone)]
struct UdasArgs {
    /// The judgment problem to explore via UDAS
    #[arg(
        value_name = "PROBLEM",
        required = true,
        trailing_var_arg = true,
    ...
    #[arg(long)]
    model: Option<String>,
    /// Comma-separated list of search angles in degrees (default: 0,120,240,60,180,300)
    #[arg(long, value_delimiter = ',')]
    angles: Option<Vec<f64>>,
    /// Maximum restorations to perform
...
/// Run the UDAS (Unitary Disk Active Search) pipeline.
///
/// Uses the full `UdasEngine` search→collapse pipeline:
/// Cold Start → Active Search → Gradient Descent → Final Collapse.
async fn run_udas(config: &Config, args: UdasArgs) -> Result<()> {
    use crate::client::DeepSeekClient;
    use deepseek_udas::engine::UdasEngine;

    let client = DeepSeekClient::new(config)?;
    let problem = join_prompt_parts(&args.problem);

    // Engine configuration
    let max_rounds = args.max_iterations.max(3);
    let cost_per_restore: usize = 2000;
    let total_budget = max_rounds * cost_per_restore;

    if !args.json {
        println!("UDAS — Unitary Disk Active Search");
        println!("Problem: {problem}");
        println!("Max rounds: {max_rounds}");
        println!("Budget: ~{total_budget} tokens (est. {cost_per_restore}/restore)");
        println!("{}", "=".repeat(60));
    }
            ...
            if args.json {
                ...
                for (i, ledger) in result.output.ledgers.iter().enumerate() {
                    println!(
                        ...
                        i + 1,
                        ledger.angle.degrees,
                        ledger.quadrant,
                        ledger.confidence.score,
                        ledger.evidence_items.len()
                    );
                    for (j, item) in ledger.evidence_items.iter().enumerate() {
...
// ─── UDAS Collapse: Memory Restoration Collision Collapse ─────────────

/// Arguments for the UDAS memory restoration collision collapse command.
///
/// This command implements the "记忆还原碰撞坍缩" workflow:
/// environmental signals → basis variations → LLM restorations →
/// interference field → contradiction resolution → collapse.
#[derive(Args, Debug, Clone)]
struct UdasCollapseArgs {
    /// The judgment problem to explore via memory restoration collision collapse
    #[arg(
        value_name = "PROBLEM",
        required = true,
        trailing_var_arg = true,
    ...
    #[arg(long)]
    model: Option<String>,
    /// User-provided context tags for additional measurement basis variations
    #[arg(long, value_name = "TAG")]
    tags: Vec<String>,
    /// Weather condition for environmental signal (e.g., "sunny", "rainy", "cloudy")
    #[arg(long, value_name = "WEATHER")]
    weather: Option<String>,
    /// Skip auto-detection of environmental signals (time, day, season)
    #[arg(long, default_value_t = false)]
    no_auto_signals: bool,
    /// Maximum restorations to perform during contradiction resolution
    #[arg(long, default_value_t = 6)]
    max_iterations: usize,
    /// Emit machine-readable JSON output
    #[arg(long, default_value_t = false)]
...
/// Run the UDAS memory restoration collision collapse pipeline.
///
/// This command implements the "记忆还原碰撞坍缩" workflow:
/// 1. Collect environmental signals (auto-detected + user tags + weather)
/// 2. Generate measurement basis variations from signals
/// 3. Perform LLM restorations at each variation (true randomness source)
/// 4. Feed measurements into the interference field
/// 5. Phase 3: Contradiction resolution at destructive interference points
/// 6. Phase 4+5: Gradient descent + collapse driven by interference pattern
///
/// The environmental signals provide genuine randomness (different times,
/// weather conditions) that creates measurement basis variations — the
/// "真随机源" for multi-angle information restoration. The interference
/// of these variations produces new information (cross terms) that drives
/// the judgment collapse.
async fn run_udas_collapse(config: &Config, args: UdasCollapseArgs) -> Result<()> {
    use crate::client::DeepSeekClient;
    use deepseek_udas::engine::UdasEngine;
    use deepseek_udas::env_signal::{
        EnvironmentalSignal, SignalType, generate_angles, generate_basis_variations,
    };
    use deepseek_udas::restoration::{self, LlmRestorer};
    ...
    let mut signals: Vec<EnvironmentalSignal> = Vec::new();

    if !args.no_auto_signals {
        let now = chrono::Local::now();

        // TimeOfDay — provides temporal context variation
        let time_str = now.format("%H:%M").to_string();
        signals.push(EnvironmentalSignal::new(SignalType::TimeOfDay, time_str));

        // DayOfWeek — provides cyclical context variation
        let day_num = now.format("%w").to_string();
        signals.push(EnvironmentalSignal::new(SignalType::DayOfWeek, day_num));

        // Season — provides seasonal context variation
    ...
    // User-specified weather condition
    if let Some(ref w) = args.weather {
        signals.push(EnvironmentalSignal::new(SignalType::Weather, w.clone()));
    }

    // User-provided context tags
    for tag in &args.tags {
        signals.push(EnvironmentalSignal::new(SignalType::UserTag, tag.clone()));
    }

    // ─── Step 2: Generate basis variations ──────────────────────────────
    let variations = generate_basis_variations(&problem, &signals);
    let angles = generate_angles(variations.len());

    if !args.json {
        println!("UDAS Collapse — Memory Restoration Collision Collapse");
        println!("Problem: {problem}");
        println!();
        ...
        println!();
        println!("── Basis Variations ({}) ──", variations.len());
        for (i, v) in variations.iter().enumerate() {
            println!("  [{}] angle={:.0}° | {}", i + 1, angles[i].degrees, v);
        }
        println!();
    ...
    if variations.len() < 3 {
        bail!(
            "Need at least 3 basis variations for MDS, got {}.              Add --tag flags or enable auto-signals.",
            variations.len()
        );
    }

    // ─── Step 3: Perform LLM restorations at each variation ─────────────
    // Each restoration uses the basis variation text (problem + env context)
    // as the "measurement basis". The LLM produces different results for
    // different bases — this is the true randomness source.
    let max_rounds = args.max_iterations.max(3);
    let cost_per_restore: usize = 2000;
    let total_budget = (variations.len() + max_rounds) * cost_per_restore;

    if !args.json {
        println!("\nPerforming {} restorations...", variations.len());
    }

    let mut measurements: Vec<MeasurementInput> = Vec::with_capacity(variations.len());

    for (i, basis_text) in variations.iter().enumerate() {
        let angle = angles[i];
        ...
        let ledger = restoration::restore(angle, basis_text, &client).await?;

        // Generate embeddings (bridge layer)
        ...
        let result_embedding = client.embed(&embed_text).await?;
        let basis_embedding = client.embed(basis_text).await?;
        let confidence = ledger.confidence.score;
        ...
        let measurement =
            MeasurementInput::new(angle, confidence, basis_embedding, result_embedding)
                .with_ledger(ledger);
        measurements.push(measurement);
    }

    // ─── Step 4: Run collision collapse pipeline ────────────────────────
    if !args.json {
        println!("\n{}", "=".repeat(60));
    ...
    match engine.run_with_measurements(measurements, Some(&problem)).await {
        Ok(result) => {
            if args.json {
                ...
                println!("\n{}", "=".repeat(60));
                println!("Memory Restoration Collision Collapse — Complete");
                println!("Measurements collected:   {}", variations.len());
                println!("Rounds executed:          {}", result.rounds);
                println!("Transitioned to GD:       {}", result.transitioned);
                println!("Final contrast ratio:     {:.3}", result.final_cr);
                println!("Peak angle:               {:.1}°", result.peak_angle.degrees);
                ...
                if let Some(ref cr) = result.contradiction_resolution {
                    println!("\n── Contradiction Resolution (Phase 3) ──");
                    println!(
                        "Destructive points found:    {}",
                        cr.destructive_points_found
                    );
                    println!(
                        "Supplementary measurements:  {}",
                        cr.supplementary_measurements
                    );
                    println!(
                        "Peak before:                 {:.1}°",
                    ...
                    println!(
                        "Peak shifted:                {}",
                        if cr.peak_shifted { "YES" } else { "NO" }
                    );

                    if cr.peak_shifted {
                        println!("\n  >> 矛盾消解方向 = 坍缩方向 <<");
                        println!("  Collapse driven by contradiction resolution");
                    }
                }
        ...
        Err(e) => {
            if args.json {
                eprintln!("UDAS collapse error: {e:#}");
            } else {
                println!("ERROR: {e:#}");
            }
        }
