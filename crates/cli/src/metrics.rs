...
//!
//! Data sources:
//! - `~/.udas/audit.log`   — one JSON line per event (approvals, credentials)
//! - `~/.udas/sessions/`   — saved session JSON files (tool call history)
//! - `~/.udas/tasks/runtime/events/` — runtime thread JSONL event streams
...
fn deepseek_home() -> PathBuf {
    // Respect UDAS_HOME env override; fall back to ~/.udas.
    if let Ok(v) = std::env::var("UDAS_HOME")
        && !v.is_empty()
    {
