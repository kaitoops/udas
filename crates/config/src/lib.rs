...
impl ConfigToml {
    /// Merge project-level overrides from `$WORKSPACE/.udas/config.toml`.
...
/// Load a project-level config from `$WORKSPACE/.udas/config.toml`.
/// Returns `None` if the file doesn't exist or can't be parsed.
pub fn load_project_config(workspace: &Path) -> Option<ConfigToml> {
    let path = workspace.join(".udas").join(CONFIG_FILE_NAME);
    if !path.exists() {
        return None;
    }
...
pub fn resolve_config_path(explicit: Option<PathBuf>) -> Result<PathBuf> {
    let path = if let Some(path) = explicit {
        path
    } else if let Ok(path) = std::env::var("UDAS_CONFIG_PATH") {
...
pub fn default_config_path() -> Result<PathBuf> {
    ...
    Ok(home.join(".udas").join(CONFIG_FILE_NAME))
}
