...
/// keyring, a JSON file under `~/.udas/secrets/`, or an in-memory
/// map (tests).
...
impl FileKeyringStore {
    ...
    pub fn default_path() -> Result<PathBuf, SecretsError> {
        ...
        Ok(home.join(".udas").join("secrets").join("secrets.json"))
    }
...
impl Secrets {
    ...
    /// [`FileKeyringStore`] under `~/.udas/secrets/`. Set
...
mod tests {
    ...
    fn auto_detect_is_file_backed_by_default() {
        ...
        assert_eq!(secrets.backend_name(), "file-based (~/.udas/secrets/)");
    }
