...
> 因为经典硬件无法使用量子算力，而作的一次折衷尝试——用几何测量，逼近人类常温量子判断力。
> UDAS（Unitary Disk Assessment System）：
> 在向量层以纯几何运算（幺正圆盘投影、密度场、坍缩、干涉场）对模型/系统判断做外部「综合征测量」；
> 以相干误差断路器、误差族鉴别、twirling 去相干等机制稀疏、正交地提取异常信号，在不坍缩主计算的前提下辅助判断力增强。

_本仓库由 DeepSeek TUI 衍化 fork 而来，承载 UDAS 判断力方法论及宿主工程。UDAS 方法论部分适用第二部分的改良署名条款（见 [LICENSE](LICENSE)）。_
...
On first launch you'll be prompted for your [DeepSeek API key](https://platform.deepseek.com/api_keys). The key is saved to `~/.udas/config.toml` so it works from any directory without OS credential prompts.

You can also set it ahead of time:

```bash
deepseek auth set --provider deepseek   # saves to ~/.udas/config.toml
...
User config: `~/.udas/config.toml`. Project overlay: `<workspace>/.deepseek/config.toml` (denied: `api_key`, `base_url`, `provider`, `mcp_config_path`). [config.example.toml](config.example.toml) has every option.
...
DeepSeek TUI discovers skills from workspace directories (`.agents/skills` → `skills` → `.opencode/skills` → `.claude/skills` → `.cursor/skills`) and global directories (`~/.agents/skills` → `~/.claude/skills` → `~/.udas/skills`). Each skill is a directory with a `SKILL.md` file:
...
`~/.udas/skills` and are versioned so new bundles are added on upgrade
